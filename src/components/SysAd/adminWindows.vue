    ...
    <div class="container">
      ...
      <div v-if="showAddModal" class="modal-box" style="padding: 1">
        <div style="display: flex; flex-direction: column">
          ...
          <div class="modalBody">
            <div class="modalBoxFormTitle">Enter a window number</div>
          ...
          <div class="modalBody">
            <div class="modalBoxFormTitle">Enter a new email address</div>
            <div class="input-group mb-3">
              <input
                type="text"
                class="form-control"
          ...
          <div class="modalBody">
            <div class="modalBoxFormTitle">Enter password</div>
            <div class="input-group mb-3">
              <input
                type="password"
                class="form-control"
      ...
      <div v-if="editWindow" class="modal-box" style="padding: 0">
        <div style="display: flex; flex-direction: column">
          ...
          <div class="modalBody" style="padding: 1rem">
            <div class="modalBoxFormTitle">Current Staff</div>
            <div class="input-group mb-3">
              <input
                type="text"
                class="form-control"
            ...
            <div class="modalBody" style="padding: 0rem">
              <div class="modalBoxFormTitle">Window #</div>
              <div class="input-group mb-3">
                <input
                  type="text"
                  class="form-control"
      ...
      <div v-if="deleteModalBox" class="deleteModalBox">
        ...
        <div class="deleteModalBody">
          <p>
            Are you sure you want to delete this window? This action cannot be
            undone and any unsaved changes or progress will be lost.
          </p>
        </div>
        <div class="line"></div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async saveChanges() {
        // Check if the new staff is selected correctly
        const { data } = await supabase
          .from("users") // Check the "users" table for the staff existence
          .select("display_name")
          .eq("display_name", this.staff);

        // If the staff doesn't exist in the "users" table, alert the user
        if (data.length === 0) {
        ...
        // Update the window's staff and window number in the "windows" table
        const { error } = await supabase
          .from("windows") // Update the "windows" table
          .update({
            current_staff: this.staff,
            window_number: this.windowNumber, // Corrected this line
          })
          .eq(
            "window_number",
            this.windowCount[this.editingIndex].windowNumber
          ); // Use the correct window number reference
        ...
        // Update the local array of windows
        this.windowCount[this.editingIndex].staff = this.staff;
        this.windowCount[this.editingIndex].windowNumber = this.windowNumber;

        // Close the modal and alert the user
        this.closeModal();
