...
<script>
  ...
  export default {
    ...
    methods: {
      async updateDatabase(day) {
        const { data, error } = await supabase
          .from("queue_dates")
          .select("id")
          .eq("date", day.formattedQueueDate);

        if (data.length === 0) {
          await supabase.from("queue_dates").insert({
            is_disabled: !day.disabled,
            date: day.formattedQueueDate,
          });
        } else {
          await supabase
            .from("queue_dates")
            .update({ is_disabled: !day.disabled })
            .eq("date", day.formattedQueueDate);
        }
