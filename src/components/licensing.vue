  ...
  <div class="licensing">
    ...
    <div class="container">
      ...
      <div class="ticketBoxes">
        <div class="ticketBox" v-for="(item, index) in paginatedTickets" :key="index">
          <p>{{ item.ticketNumber }}</p>
          <p>{{ item.service }}</p>
          <p>{{ item.reference }}</p>
          <div class="statusBox">
            <button class="statusButton" :class="getStatusClass(item.status)" @click.stop="toggleDropdown(index)">
              {{ item.status }}
            </button>
    ...
    <div v-if="showModal" class="modal-box">
      ...
      <div class="modalBody">
        <p>
          Are you sure you want to complete this ticket? This action cannot be
