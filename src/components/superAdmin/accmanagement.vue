  ...
  <div class="accmanagement">
    ...
    <div class="container">
      ...
      <div class="ticketBoxes">
        <div class="ticketBox" v-for="(item, index) in accountCount" :key="index">
          <p>{{ index + 1 }}</p>
          <p>{{ item.email }}</p>
          <p>{{ item.role }}</p>
          <div class="statusBox">
      ...
      <div v-if="showModal" class="modal-box" style="padding: 0">
        <div style="display: flex; flex-direction: column">
          ...
          <div class="modalBody" style="padding: 1rem">
            ...
            <div class="input-group mb-3">
              <div class="dropdown">
                <input type="button" class="form-control" :value="role || 'Select Role'" @click="toggleRoleDropdown" />
                <div v-if="showRoleDropdown" class="dropdown-menu" @click.stop>
                  <p v-for="roleOption in ['staff', 'system admin']" :key="roleOption" @click="selectRole(roleOption)"
                    class="dropdown-item">
                    {{ roleOption }}
                  </p>
                </div>
      ...
      <div v-if="showEditAccount" class="modal-box" style="padding: 0">
        <div style="display: flex; flex-direction: column">
          ...
          <div class="modalBody" style="padding: 1rem">
            ...
            <div class="input-group mb-3">
              <input type="email" class="form-control" placeholder="Enter New Email" aria-label="Email Address"
                aria-describedby="basic-addon1" v-model="email" />
            </div>
            <div class="modalBoxFormTitle">
              Current Role
      ...
      <div v-if="deleteModalBox" class="deleteModalBox">
        ...
        <div class="deleteModalFooter">
          ...
          <button @click="deleteAccount" class="btn btn-primary specbtn">
            Confirm
          </button>
        </div>
      </div>
